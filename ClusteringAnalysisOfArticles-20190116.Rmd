...
clusterByTopic.df=reshape2::melt(clusterByTopic.df, id.vars = c("id.cluster"), variable.name = "topic", value.name = "topic.weight")

top.topics.by.cluster <- clusterByTopic.df %>%
  group_by(id.cluster) %>%
  top_n(10, topic.weight) %>%
  ungroup() %>%
  arrange(id.cluster, desc(topic.weight))

top.topics.by.cluster %>%
  mutate(topic = reorder(topic, topic.weight)) %>%
  ggplot(aes(topic, topic.weight, fill = factor(id.cluster))) +
