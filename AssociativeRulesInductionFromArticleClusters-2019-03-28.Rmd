...
###save results by threshold by cluster
```{r}
...
```{r}
selected.rules=rules.interest #use rule object to analys all rules

library("future.apply")
plan(multiprocess, workers = 7)
k=kmeanDocModel$size %>% length(.)
valid.support.doc.by.rule=future_lapply((1:k), function(x){
  idRuleWithconsequent=selected.rules@rhs %in% as.character(x)
  rules.for.k=selected.rules[idRuleWithconsequent]
  supportTransactionOfCluster=which(kmeanDocModel$cluster==x)
...
xxx=rbindlist(doc.to.rule.with.info)

######################################
#seems that on very large list, rbindlist needs to be called several times on splited lists.
#In these cases, the code below will do the job
...
###save
```{r}
write.csv(x = xxx, file = paste0("valide.supporting.document.of.rulesOfInterest96.", Sys.Date(), ".csv"), sep = ",", fileEncoding = "UTF-8")
```


##Retreive all documents for selected rules 
