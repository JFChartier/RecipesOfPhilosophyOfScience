...
set.seed(123)
rpartTree <- rpart(cluster ~ ., data = ArtAndTopicDiscr.df, method = "class", control = rpart.control(minbucket=10, maxdepth=30, cp = 00, xval=100), parms = list(split="information"))
...
rpart.cv.fit <- train(y = y, x = x, method = "rpart", trControl = data_ctrl,parms=list(split='information'), tuneGrid = data.frame(cp = c(min.cp)), control = rpart.control(minsplit = 10, minbucket = 10))
...
rpartTree.full <- rpart(cluster ~ ., data = ArtAndTopicDiscr.df, method = "class", control = rpart.control(minbucket=10, maxdepth=30, cp = min.cp, xval=1), parms = list(split="information"))
